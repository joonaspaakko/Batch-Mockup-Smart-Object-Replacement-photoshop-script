...
function replaceLoop(data) {
  var hrznOffsets = [0, 1125, -1125]; // required horizantal offset values (each value will be exported as a seperate image)
      ...
      if (item.target) {
        ...
        if (targetConfirmed) {
          ...
          if (sourceFilePath !== null) {
            ...
            for (
              var hrznOffsetIndex = 0;
              hrznOffsetIndex < hrznOffsets.length;
              hrznOffsetIndex++
            ) {
              var hrznOffset = hrznOffsets[hrznOffsetIndex];
              // pass hrznOffsets values to replaceSoContents()
              replaceSoContents(item, sourceFilePath, hrznOffset); // The old switcheroo

              var outputPathPrefix = data.output.path + "/";
              var outputFilePath = parseFilePath(
                data,
                fileIndex,
                outputPathPrefix,
                hrznOffsetIndex
              ...
              app.activeDocument.saveAs(
                new File(outputFilePath.fullpath),
                saveOpts()[data.output.format](),
                true,
                Extension.LOWERCASE
              );
            }
...
function parseFilePath(data, fileIndex, outputPathPrefix, hrznOffsetIndex) {
  var fileNumber = fileIndex + 1;
  if (data.output.zeroPadding)
    fileNumber = zeroPadding(fileNumber, data.maxLoop.toString().length);
  ...
  var filename = data.output.filename
    .replace(/@mockup/g, inputFilename)
    .replace(/\$/g, hrznOffsetIndex + 1)
    .replace(/@input/g, inputFilename);
   // Just a thought....
...
function getFiles(folder, item) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    var regex =
      ...
      (item.inputFormats
        ? item.inputFormats
        : "tiff?|gif|jpe?g|bmp|eps|svg|png|ai|psd|pdf") +
      ")$";
...
function replaceLoopOptionsFiller(rawData) {
  ...
  var data = {
    output: {
      ...
      filename: rawData.output.filename || "@mockup_$",
      zeroPadding:
        rawData.output.zeroPadding === undefined
          ? true
          : rawData.output.zeroPadding,
    },
  };
  ...
  for (var i = 0; i < rawData.items.length; i++) {
    ...
    itemObj.resize =
      rawItem.resize || rawItem.resize === false ? rawItem.resize : "fill";
    if (rawItem.inputPlaced_runAction)
      itemObj.inputPlaced_runAction = rawItem.inputPlaced_runAction;
    if (rawItem.inputPlaced_runScript)
      itemObj.inputPlaced_runScript = rawItem.inputPlaced_runScript;
    items.push(itemObj);
  }
  ...
  for (var i = 0; i < data.items.length; i++) {
    ...
    for (var inputIndex = 0; inputIndex < item.input.length; inputIndex++) {
      item.input[inputIndex] = absolutelyRelativePath(
        item.input[inputIndex]
      ).decoded;
    }
  }
  ...
  if (data.output.folders) {
    data.output.path = absolutelyRelativePath(
      data.output.path + "/" + data.doc.name
    ).decoded;
    newFolder(data.output.path);
  }
...
function replaceSoContents(item, sourcepath, hrznOffset) {
  // EDIT SO CONTENTS
  try {
    executeAction(
  ...
  if (editingSO) {
    ...
    if (file.exists) {
      try {
        ...
        // where hrznOffset value is used
        desc395.putUnitDouble(idHrzn, idPxl, hrznOffset);
        var idVrtc = charIDToTypeID("Vrtc");
        var idPxl = charIDToTypeID("#Pxl");
        desc395.putUnitDouble(idVrtc, idPxl, 0.0);
        var idOfst = charIDToTypeID("Ofst");
        ...
        if (item.align)
          align(
            doc.activeLayer,
            [0, 0, doc.width.value, doc.height.value],
            item.align
          );
        ...
        try {
          if (item.inputPlaced_runAction) {
            app.doAction(
              item.inputPlaced_runAction[1],
              item.inputPlaced_runAction[0]
            );
          }

          if (item.inputPlaced_runScript) {
            const scriptPath = absolutelyRelativePath(
              item.inputPlaced_runScript
            ).decoded;
  ...
  if (!ignoreBackground && detachBG) {
    try {
      ...
      if (bgVisible) {
        ...
        desc305.putUnitDouble(idOpct, idPrc, 100.0);
...
function calculateNewSize(inputSize, targetSize) {
  ...
  return {
    pixels: {
      ...
      fill: {
        width: inputSize[0] * ratioFill,
        height: inputSize[1] * ratioFill,
      },
...
function align(imageLayer, targetBounds, alignment) {
  var imageBounds =
    imageLayer.boundsNoEffects || app.activeDocument.activeLayer.bounds;
...
function absolutelyRelativePath(string) {
  ...
  if (familyTree) {
    ...
    // Traverse upwards
    var pathPrefix = new File(
      familyTree.type === "doc" ? docPath : includePath
    );
    each(familyTree.array, function () {
